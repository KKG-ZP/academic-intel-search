#!/usr/bin/env bash
# Algorithm/methodology domain search via Semantic Scholar with CrossRef fallback
# Usage: bash scripts/algo_search.sh "query" [year_range] [limit]
...
if [[ -z "$QUERY" ]]; then
  echo '{"error":"Usage: bash scripts/algo_search.sh \"query\" [year_range] [limit]"}' >&2
  exit 1
fi
...
enhanced_query="$QUERY transformer diffusion foundation model time series self-supervised learning"
  ...
  echo "$body" | jq --arg threshold "$ARXIV_THRESHOLD" '
    [
       ...
       else "normal" end) as $arxiv_status |
      {
        ...
        venue: $venue,
        venue_tier: (
          if ($venue | test("(?i)NeurIPS|ICML|ICLR|AAAI|KDD|CVPR|ICCV|ECCV")) then "top_ml"
          elif ($venue | test("(?i)Nature Machine Intelligence|IEEE.*Signal Processing|JMLR|TPAMI")) then "core"
          else "other"
          end
        ),
        citations: $cites,
