#!/usr/bin/env bash
# Neuro-modulation domain search via Semantic Scholar with CrossRef fallback
# Usage: bash scripts/neuro_search.sh "query" [year_range] [limit]
...
if [[ -z "$QUERY" ]]; then
  echo '{"error":"Usage: bash scripts/neuro_search.sh \"query\" [year_range] [limit]"}' >&2
  exit 1
fi
...
enhanced_query="$QUERY neuromodulation neural stimulation brain stimulation ultrasound"
  ...
  echo "$body" | jq --arg threshold "$ARXIV_THRESHOLD" '
    [
       ...
       else "normal" end) as $arxiv_status |
      {
        ...
        venue: $venue,
        venue_tier: (
          if ($venue | test("(?i)Nature|Science|PNAS|Cell|Brain Stimulation|Nature Neuroscience|Nature Communications")) then "top"
          elif ($venue | test("(?i)Ultrasound|IEEE.*Ultrasonics|Neuromodulation|Journal of Neural Engineering")) then "core"
          else "other"
          end
        ),
        citations: $cites,
